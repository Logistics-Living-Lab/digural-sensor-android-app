...
import de.digural.app.util.RetryException
import de.digural.app.util.RetryHelper.Companion.runWithRetry
...
class MqttManager(
    ...
    private suspend fun connect(): Flow<Boolean> = callbackFlow<Boolean> {
        try {
            ...
            mqttConnectOptions.userName =
                JWT(accessToken)
                    .getClaim(de.digural.app.AppConstants.AUTH_USERNAME_CLAIM_KEY)
                    .asString()
    ...
    private fun createMqttClient(): MqttAsyncClient {
        val mqttClient =
            MqttAsyncClient(
                de.digural.app.AppConstants.MQTT_SERVER_URL,
